...
import { generateAdminToken, parseAdminApiKey } from '../src/lib/auth.js';

describe('auth key parsing', () => {
  test('parses valid admin key', () => {
    const parsed = parseAdminApiKey('abc123:0011aabb');
    expect(parsed.id).toBe('abc123');
    expect(parsed.secretHex).toBe('0011aabb');
  });

  test('rejects invalid key', () => {
    expect(() => parseAdminApiKey('invalid')).toThrowError();
  });
